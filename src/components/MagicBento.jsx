...
// Helper to create the visual particles
...
const ParticleCard = ({
  ...
  useEffect(() => {
    ...
    const handleClick = e => {
      ...
      ripple.style.cssText = `position: absolute; width: ${maxDistance * 2}px; height: ${maxDistance * 2}px; border-radius: 50%; background: radial-gradient(circle, rgba(${glowColor}, 0.4) 0%, transparent 70%); left: ${x - maxDistance}px; top: ${y - maxDistance}px; pointer-events: none; z-index: 1000;`;
  ...
  useEffect(() => {
    ...
    const handleMouseMove = e => {
      ...
      cards.forEach(card => {
        ...
        const distance = Math.hypot(e.clientX - (cardRect.left + cardRect.width / 2), e.clientY - (cardRect.top + cardRect.height / 2));
              ...
              <div className="magic-bento-card__content">
                  ...
                  <div className="flex flex-wrap gap-2 mt-4">
                    {card.skills.map(skill => (
                      <span key={skill.name} className="flex items-center gap-1.5 px-3 py-1 text-xs font-medium bg-white/5 border border-white/10 rounded-full text-white/80 backdrop-blur-sm">
                        <img src={`https://skillicons.dev/icons?i=${skill.slug}`} alt={skill.name} className="w-3.5 h-3.5 object-contain" />
                        {skill.name}
                      </span>
                    ))}
                  </div>
                ...
                {card.links && (
                  <div className="flex gap-4 mt-4 relative z-20 pointer-events-auto">
                    {card.links.map(link => (
                      <a key={link.name} href={link.url} target="_blank" rel="noreferrer" className="text-white/60 hover:text-white transition-all duration-300 p-2.5 bg-white/5 rounded-full hover:bg-white/15 hover:scale-110 border border-white/5" title={link.name}>
                        {Icons[link.icon]}
                      </a>
                    ))}
                  </div>
